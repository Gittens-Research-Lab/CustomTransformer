...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, edge_bias_k=None, edge_bias_v=None, mask=None):
        ...
        scores = attention(q, k, v, self.d_k, edge_bias_k, edge_bias_v, mask, self.dropout) # [b, nh, seq_len, d]
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, edge_bias_k=None, edge_bias_v=None, mask=None):
        x2 = self.norm_1(x)
        x = x + self.dropout_1(self.attn(x2,x2,x2, edge_bias_k, edge_bias_v, mask))
        x2 = self.norm_2(x)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, edge_bias_k=None, edge_bias_v=None, mask=None):
        ...
        for i in range(self.nl):
            x = self.layers[i](x, edge_bias_k, edge_bias_v, mask)
        return self.norm(x)